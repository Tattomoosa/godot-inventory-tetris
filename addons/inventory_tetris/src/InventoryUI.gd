...
var picked_item_instance : InventoryItemInstance:
	get:
		if !active_inventory.picked_item_instance:
			return null
		return active_inventory.picked_item_instance.item_instance
...
func _on_picked_item_changed():
	...
	if !picked_item_instance:
		item_info_panel.item = null
		return
	item_info_panel.item = picked_item_instance.item
...
func _disconnect_inventory_signals(inventory: InventoryGridPanel):
	...
	if inventory.picked_item_instance_changed.is_connected(_on_picked_item_changed):
		inventory.picked_item_instance_changed.disconnect(_on_picked_item_changed)
...
func _connect_inventory_signals(inventory: InventoryGridPanel):
	...
	inventory.picked_item_instance_changed.connect(_on_picked_item_changed)
